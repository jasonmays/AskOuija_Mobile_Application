...
const AnsweringSpirit = (props) => {
  ...
  const addLetter = () => {
    return firestore()
      ...
      .then((response) => {
        ...
        while (
          newAnsweringSpirit.uid === props.GameData.answeringSpirit.uid ||
          newAnsweringSpirit.uid === props.GameData.questionAsker.uid
        ) {
      ...
      .catch((err) => {
        let friendlyError = {
          friendly: 'Something has gone horribly wrong.',
  ...
  const finalizeAnswer = () => {
    ...
    return firestore()
      ...
      .catch((err) => {
        let friendlyError = {
          friendly: 'Something has gone terribly wrong.',
          technical: err.toString(),
        };
        setError(() => {
          throw friendlyError;
  ...
  const getRandomPlayer = () => {
    return props.GameData.players[
      Math.floor(
        Math.random() * (props.GameData.players.length - 1 - 0 + 1) + 0,
      )
    ];
  };
      ...
      <View style={props.styles.aoLobbyContainer}>
        <View style={props.styles.aoLobbyInnerContainer}>
          <Text style={props.styles.aoHeadline}>
            ...
            <Text style={{color: '#ECA72C'}}>{props.GameData.question}</Text>
          </Text>
          <View style={props.styles.aoAnswerSpace}>
            ...
            <View style={props.styles.aoAnswerLetterTile}>
              <Text style={props.styles.aoAnswerLetterTileText}>{'_'}</Text>
            </View>
          </View>
        </View>
        <View
          style={{
            ...props.styles.aoSpiritWatchingGameBar,
          ...
          <ScrollView
            style={props.styles.aoLetterBar}
              ...
              <TouchableOpacity
                ...
                style={
                  selectedLetter === letter
                    ? {
                        ...props.styles.aoAnswerLetterTileSelected,
                        marginVertical: 0,
                      }
                    : {...props.styles.aoAnswerLetterTile, marginVertical: 0}
                }
                onPress={() => toggleLetter(letter)}>
                <Text
                  style={
                    selectedLetter === letter
                      ? props.styles.aoAnswerLetterTileSelectedText
                      : props.styles.aoAnswerLetterTileText
                  }>
                  {letter}
                </Text>
              </TouchableOpacity>
          ...
          <TouchableOpacity
            style={
              selectedLetter !== ''
                ? props.styles.aoPrimaryButton
                : props.styles.aoPrimaryButtonDisabled
            }
            onPress={() => addLetter()}>
            <Text style={props.styles.aoPrimaryButtonText}>{'Add Letter'}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={
              selectedLetter === ''
                ? props.styles.aoSecondaryButton
                : props.styles.aoSecondaryButtonDisabled
            }
            onPress={() => finalizeAnswer()}>
            <Text
              style={
                selectedLetter === ''
                  ? props.styles.aoSecondaryButtonText
                  : props.styles.aoSecondaryButtonDisabledText
              }>
              {'Finalize Answer'}
            </Text>
          </TouchableOpacity>
