...
const Lobby = (props) => {
  ...
  const startGame = () => {
    if (
      ...
      props.GameData.owner === props.auth.uid &&
      props.GameData.players.length > 2
    ) {
      ...
      return setRoles(questionAsker, answeringSpirit).then((response) => {
        if (response.hasError) {
          let friendlyError = {
            friendly: 'Something has gone terribly wrong.',
            technical: response.value.toString(),
          };
          setError(() => {
            throw friendlyError;
          });
    ...
    <View style={props.styles.aoGameContainer}>
      <View style={props.styles.aoGameInnerContainer}>
        <View style={props.styles.aoLobbyContainer}>
            ...
            <>
              {props.GameData.players.map((player, index) => (
                <View key={index} style={props.styles.aoPlayerRow}>
                  <Image
                    source={{uri: 'data:image/png;base64, ' + player.avatar}}
          ...
          <Text
            style={{...props.styles.aoText, marginTop: 12, marginBottom: 24}}>
            {props.GameData.players.length < 8
