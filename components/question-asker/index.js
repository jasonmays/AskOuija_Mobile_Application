...
const QuestionAsker = (props) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      props.GameData.questionAsker.uid === props.auth.uid &&
      ...
      const getQuestions = async () => {
        let someQuestions = [];
        while (someQuestions.length < 3 && counter.current < 300) {
          ...
          <View style={props.styles.aoLobbyContainer}>
            {!selectedGenre ? (
                  ...
                  <View
                    ...
                    {genres.map((genre) => (
                      <TouchableOpacity
                        style={
                          selectedGenre === genre
                            ? props.styles.aoQuestionRowSelected
                            : props.styles.aoQuestionRow
                        }
                        key={genre}
                        onPress={() => {
                          setSelectedGenre(genre);
                        }}>
                        <Text style={props.styles.aoQuestionText}>{genre}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </View>
              ...
              <>
                  ...
                  <View
                    ...
                    {questions.map((question, index) => (
                      <TouchableOpacity
                        ...
                        key={index}
                        onPress={() => selectQuestion(index, question)}>
                    ...
                    <TextInput
                      style={{
                        ...props.styles.aoTextbox,
                        marginHorizontal: 12,
                        width: '100%',
                      }}
                      value={customQuestionText}
                      onChangeText={(text) => doCustomQuestion(text)}
                ...
                <TouchableOpacity
                  style={
                    selectedQuestion.id !== -1 || customQuestionText.length > 0
