...
const App: () => React$Node = () => {
  ...
  const joinGame = (gameDocID) => {
    ...
    return getAvatar().then((avatar) => {
      if (!avatar.hasError) {
        return getPlayers(gameDocID).then((players) => {
          if (!players.hasError) {
            ...
            if (!alreadyThere) {
              ...
              return updatePlayers(gameDocID, newPlayers).then((response) => {
            ...
            let friendlyError = {
              friendly: 'Something has gone terribly wrong.',
              technical: players.value !== undefined ? players.value : '',
            };
            setError(() => {
              throw friendlyError;
            });
        ...
        <ErrorBoundary style={styles}>
          {(() => {
            if (
              ...
              gameData.questionAsker.uid === auth().currentUser.uid
            ) {
              return (
                <QuestionAsker
                  styles={styles}
            ...
            if (
              gameData !== undefined &&
              gameData.status === 'playing' &&
              gameData.answeringSpirit.uid === auth().currentUser.uid
            ) {
              return (
                <AnsweringSpirit
                  styles={styles}
